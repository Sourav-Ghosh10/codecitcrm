...
class UserController {
  ...
  static refreshToken = async (req, res) => {
    ...
    if (!refreshTokens.includes(token)) return res.status(403).json({ error: "Refresh token is not valid" });

    jwt.verify(token, REFRESH_TOKEN_SECRET, (err, user) => {
      if (err) return res.status(403).json({ error: "Refresh token is invalid or expired" });

      const accessToken = generateAccessToken({ menu: user.menu, id: user.id });
      // Optional: generate new refresh token and remove old one (token rotation)
      // refreshTokens = refreshTokens.filter(t => t !== token);
      // const newRefreshToken = generateRefreshToken({ menu: user.menu, id: user.id });
